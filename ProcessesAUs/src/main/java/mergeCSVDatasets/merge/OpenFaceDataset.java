package mergeCSVDatasets.merge;

import mergeCSVDatasets.utils.DateTimeUtils;
...
public class OpenFaceDataset {
    ...
    public OpenFaceDataset(String openFaceDatasetsFolderPath, String pTargetFilePath) {
        this.fileUtils = new FileUtils();
        this.mergeOpenFaceOutputs(openFaceDatasetsFolderPath, pTargetFilePath);

        /*if(!fileTemp.exists())
            fileTemp = defaultOpenFaceOutput();*/
        this.outputFile = new File(pTargetFilePath);
    }
    ...
    private void mergeOpenFaceOutputs(String openFaceDatasetsFolderPath, String pTargetFilePath) {
        File[] files;
        files = fileUtils.getFilesFromFolder(openFaceDatasetsFolderPath, "csv");
        ...
        if (files != null) {
            ...
            for (File openFaceFile : files) {
                List<String[]> list = this.fileUtils.parseCSVFile(openFaceFile.toString());
            ...
            fileUtils.writeFile(pTargetFilePath, newOpenFaceList, false);
        }
    }
    ...
    public List<String[]> getOpenFaceAUs() {
        List<String[]> newOpenFaceList = new ArrayList<>();
        String[] newLine;
        List<String[]> openFaceOutput = this.fileUtils.parseCSVFile(this.outputFile.toString());

        for (int i = 1; i < openFaceOutput.size(); i ++) {  //position 0 is the header
            newLine = getNewOFLine(outputFile, openFaceOutput, i);
            newOpenFaceList.add(newLine);
        }
